...
const CookiePolicy = () => {
  const [data, setData] = useState({ title: "Cookie Policy", lastUpdated: "", content: "" });
  ...
  useEffect(() => {
    const fetchCookies = async () => {
      try {
        setLoading(true);
        const response = await api.get("/legal/cookie-policy", { withCredentials: true });
        if (response.data.success) setData(response.data);
      } catch (err) {
        if (err.response?.status === 401) {
          setError("Authentication required. Please log in.");
        } else {
          setError(err.response?.data?.message || "Unable to load Cookie Policy.");
        }
      } finally {
        setLoading(false);
      }
    ...
    <main className="container legal-page" style={{ padding: "100px 20px", lineHeight: "1.8" }}>
      {error ? (
        ...
        <>
          ...
          <Link to="/home" className="btn-submit" style={{ marginTop: "2rem", display: "inline-block", textDecoration: 'none' }}>Got it</Link>
        </>
      )}
    </main>
  );
