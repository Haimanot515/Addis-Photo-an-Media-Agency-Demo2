...
import { Scale, Save, Trash2, Hash, Plus, RefreshCcw } from 'lucide-react';
...
const AdminTermsOfServices = () => {
    ...
    const syncRegistry = async () => {
        ...
        try {
            ...
            setSections(res.data.registry.terms.sort((a, b) => a.serial_number - b.serial_number));
    ...
    const pushNode = async (index) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.put('/api/admin/legal/terms', node, { headers: { Authorization: `Bearer ${token}` } });
            alert(`TERMS NODE ${node.serial_number} COMMITTED`);
            syncRegistry();
        } catch (err) { alert("PUSH_ERROR"); }
    };

    const dropNode = async (id, index) => {
        if (!window.confirm("EXECUTE DROP: REMOVE THIS TERMS RULE?")) return;
        try {
            const token = localStorage.getItem('token');
            if (id) await axios.delete(`/api/admin/legal/terms/${id}`, { headers: { Authorization: `Bearer ${token}` } });
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm mb-8">
            <div className="flex justify-between mb-6">
                <div className="flex items-center gap-3">
                    <div className="bg-emerald-50 p-3 rounded-2xl text-emerald-600"><Scale size={24} /></div>
                    <h2 className="text-sm font-black uppercase tracking-widest">Terms Authority</h2>
                </div>
                <div className="flex gap-2">
                    ...
                    <button onClick={addNode} className="bg-[#1b1b2f] text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-lg"><Plus size={16} className="inline mr-2"/> Add Rule</button>
                </div>
            </div>
                ...
                {sections.map((node, index) => (
                    <div key={index} className="grid grid-cols-12 gap-3 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                        ...
                        <div className="col-span-2 flex gap-2 justify-end">
                            <button onClick={() => pushNode(index)} className="p-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 shadow-md shadow-emerald-100"><Save size={16} /></button>
